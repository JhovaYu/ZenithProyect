...
export class PerfilPage implements OnInit {
  ...
  loadUserProfile() {
    ...
    if (userInfo) {
      this.studentName = userInfo.nombre || '';
      this.studentLastName = userInfo.apellido || '';
      this.studentEmail = userInfo.email || '';
      this.studentMatricula = userInfo.matricula || '';

      this.profileForm.patchValue({
        nombre: this.studentName,
        apellido: this.studentLastName,
  ...
  toggleEdit(field: string) {
    ...
    // Los campos de correo electrónico y matrícula siempre permanecen deshabilitados
    this.profileForm.get('email')?.disable();
    this.profileForm.get('matricula')?.disable();
  }
  ...
  onSubmit() {
    if (this.profileForm.valid) {
      ...
      this.http.put(`${environment.apiUrl}/student/profile`, updatedProfile, {
